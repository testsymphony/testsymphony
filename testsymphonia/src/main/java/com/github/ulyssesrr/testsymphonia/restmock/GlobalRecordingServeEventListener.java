...
public class GlobalRecordingServeEventListener implements ServeEventListener {
    ...
    public void startRecording(String scenarioId) {
        recordings.put(scenarioId, new ConcurrentLinkedQueue<>());
    }

    public List<StubMapping> stopRecording(String scenarioId) {
        List<ServeEvent> serveEvents = new ArrayList<>(recordings.remove(scenarioId));
        ...
        recordedStubMappings.forEach(stubMapping -> {
            stubMapping.setScenarioName(scenarioId);
        });
