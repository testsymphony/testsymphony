...
public class CorrelationRecordingServeEventListener implements ServeEventListener {
    ...
    public void afterComplete(ServeEvent serveEvent, Parameters parameters) {
        ...
        String scenarioId = serveEvent.getRequest().getHeader("X-Correlation-ID");
        if (scenarioId != null) {
            recordings.computeIfPresent(scenarioId, (k, v) -> {
                v.add(serveEvent);
                log.info("RECORDED: "+scenarioId+" | Event: "+serveEvent);
                return v;
            });
        }
    }
    ...
    public void startRecording(String scenarioId) {
        recordings.put(scenarioId, new ArrayList<>());
    }

    public List<StubMapping> stopRecording(String scenarioId) {
        List<ServeEvent> serveEvents = recordings.remove(scenarioId);
