...
public class TestSymphonyAgentInstrumentation {

    private static final StdioLogger logger = StdioLogger.INSTANCE;
    
    public static void instrumentation(String agentArgs, Instrumentation instrumentation, ClassFileLocator compoundLocator) {
        logger.info("Initializing TestSymphony Agent V2");
        
        try {
            // Load configuration
            AgentConfig config = AgentConfig.load().propagateToSystemProps();
